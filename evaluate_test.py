...
parser.add_argument("--y_test_data_path", default='./image',
...
parser.add_argument("--out_dir_y", default='./test_output_yyyy/', help="Output Folder")  # 保存y域的输入图片与生成的x域图片的路径

args = parser.parse_args(args=[])


def make_test_data_list(y_data_path):  # make_test_data_list函数得到测试中的x域和y域的图像路径名称列表
    y_input_images = glob.glob(os.path.join(y_data_path, "*"))  # 读取全部的y域图像路径名称列表
    return y_input_images
...
def get_write_picture(fake_x):  # get_write_picture函数得到网络测试结果
    # y_image = cv_inv_proc(y_image)  # 还原y域的图像
    fake_x = cv_inv_proc(fake_x[0])  # 还原生成的x域的图像
    y_output = np.concatenate(fake_x, axis=1)  # 得到y域的输入图像以及对应的生成的x域图像
...
def main():
    ...
    tf.reset_default_graph()  # 还原模型，二次使用
    y_datalists = make_test_data_list(args.y_test_data_path)  # 得到待测试的x域和y域图像路径名称列表
    ...
    total_step = len(y_datalists)  # 测试的总步数
    for step in range(total_step):
        ...
        feed_dict = {test_y_image: batch_y_image}  # 建立feed_dict
        fake_x_value = sess.run([fake_x], feed_dict=feed_dict)  # 得到生成的y域图像与x域图像
        y_write_image = get_write_picture(fake_x_value)  # 得到最终的图片结果
        ...
        pic = cv2.imread(y_write_image_name)  # 读入保存的图片
        pic = cv2.resize(pic, (200, 250), interpolation=cv2.INTER_CUBIC)  # 处理图片长宽比
        cv2.imwrite(y_write_image_name, pic)  # 将处理好的图片重新保存
